...
#### `jnote(title: none, date: none, affiliation: none, author: none, dark: false, doc)`
数学文書全体を包むメイン関数です。
- **引数**:
  - `title` - 文書のタイトル（オプション）
  - `date` - 文書の日付（オプション）
  ...
  - `author` - 著者名（オプション）
  - `dark` - ダークモード（`true`で有効、既定は`false`）
  - `doc` - 文書の内容
- **機能**: 
  ...
  - 図表番号を章番号付きで管理（1.1形式）
  - 章が変わる際にカウンターをリセット
  - `dark: true`で背景を暗色・文字を明色にし、各コールアウト（定義・定理等）の背景色と枠線色をダークモード向けに切り替え

## ダークモード

`jnote(dark: true, ...)`でダークモードを有効にできます。
ページ背景・本文の色に加え、`definition`〜`example`の各ボックスの
背景色と枠線色がダークモード向けに自動で調整されます。

```typst
#import "@original/jnote:1.0.2": *
#show: doc => jnote(title: "ノート", dark: true, doc)
